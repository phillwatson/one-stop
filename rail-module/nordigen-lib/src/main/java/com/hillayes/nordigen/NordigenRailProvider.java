    ...
    public Optional<RailAccount> getAccount(RailAgreement agreement, String id) {
        ...
        return accountService.get(id)
            .map(account -> RailAccount.builder()
                .id(account.id)
                .iban(Strings.getOrDefault(account.bban, account.iban))
            ...
            .map(account -> accountService.details(id)
                ...
                .map(accountProperties -> account.toBuilder()
                    .name(accountProperties.getOrDefault("name", accountProperties.getOrDefault("details", account.getName())))
                    .iban(accountProperties.getOrDefault("maskedPan", account.getIban()))
        ...
        return accountService.transactions(accountId, dateFrom, LocalDate.now())
            .map(transactions -> transactions.booked.stream()
                .map(transaction -> RailTransaction.builder()
                    ...
                    .reference(Strings.getOrDefault(transaction.entryReference, transaction.additionalInformation))
                    .description(Strings.toStringOrNull(transaction.remittanceInformationUnstructured))
    ...
    private Optional<RailBalance> getBalance(String accountId) {
        ...
        return accountService.balances(accountId)
            .flatMap(balances -> balances.stream()
                ...
                .findFirst()
                .or(() -> balances.stream()
                    // caters for some credit cards (e.g. natwest)
                    .filter(balance -> "forwardAvailable".equals(balance.balanceType))
                    .findFirst()
                )
